...
pub struct ExcodeArgs {
    ...
    /// Model used by the interactive coding agent. With the global
    /// `--harness` flag, this must name a registered model binding (see
    /// `exo model register`); when unset, the first registered binding is used.
    #[arg(long)]
